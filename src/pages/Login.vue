<template>
    <div class="flex flex-col items-center space-y-4 mt-10">
        ...
        placeholder="이메일" />
       <input v-model="password" type="text" class="rounded w-96 px-4 py-3 border border-gray-300 focus:ring-2 focus:border-primary focus:outline-none"
        placeholder="비밀번호" />
        <button class="w-96 rounded bg-primary text-white py-4 hover:bg-dark" @click="onLogin">회원가입</button>
...
export default {
    setup() {
        ...
        const onLogin = () => {
            console.log( email.value, passowrd.value)
        }
